...
const handleFileUploading = (file, uiniqueIndentifier) => {
	...
	//const APLI_URL = '../server/app';
	formData.append('file', file);

	xhr.upload.addEventListener('progress', (event) => {
		console.log(event.loaded);
		//uploading progres bar and file size element
		const fileProgres = document.querySelector(
			`#file-item-${uiniqueIndentifier} .file-progress`,
		);
		const fileZise = document.querySelector(
			`#file-item-${uiniqueIndentifier} .file-size`,
		);
		...
		const formattedFileSize =
			file.size >= 1024 * 1024
				...
				: `${(event.loaded / 1024).toFixed(2)} KB / ${(
						event.total / 1024
				  ).toFixed(2)} KB`;
...
fileUploadBox.addEventListener('drop', (event) => {
	...
	fileUploadBox.querySelector('.file-instruction').textContent =
		'Drag files here or';
});
